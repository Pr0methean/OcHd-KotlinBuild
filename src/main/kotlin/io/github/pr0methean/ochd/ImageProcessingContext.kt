...
class ImageProcessingContext(
    ...
    private val pngTranscoder = PNGTranscoder()
    init {
        pngTranscoder.addTranscodingHint(ImageTranscoder.KEY_HEIGHT, tileSize)
        svgDirectory.list()!!.forEach { svgFile ->
            svgTasks[svgFile.removeSuffix(".svg")] = SvgImportTask(svgDirectory.resolve(svgFile), pngTranscoder, scope)
        }
    }
    ...
    fun layer(name: String, paint: Paint? = null, alpha: Double = 1.0): TextureTask {
        val importTask = SvgImportTask(svgDirectory.resolve("$name.svg"), pngTranscoder, scope)
    ...
    fun stack(background: Paint = Color.TRANSPARENT, init: LayerList.() -> Unit): TextureTask {
        val layerTasks = LayerList(this)
        layerTasks.init()
        return deduplicate(
            if (layerTasks.size == 1 && background == Color.TRANSPARENT)
                layerTasks[0]
            else
                ImageStackingTask(layerTasks.toList(), tileSize, scope))
    }
