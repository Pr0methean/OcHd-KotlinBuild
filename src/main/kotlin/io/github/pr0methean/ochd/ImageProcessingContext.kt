...
class ImageProcessingContext(
    ...
    suspend fun deduplicate(task: Task<Image>): ImageTask {
        ...
        if (task !is ImageTask) {
            ...
            return object: AbstractImageTask(task.name, noopTaskCache(), stats) {
    ...
    suspend fun layer(source: Task<Image>, paint: Paint? = null, alpha: Double = 1.0): ImageTask {
        return deduplicate(RepaintTask(deduplicate(source), paint, alpha, noopTaskCache(), stats))
    }
    ...
    suspend fun stack(init: suspend LayerListBuilder.() -> Unit): ImageTask {
        ...
        return deduplicate(ImageStackingTask(layerTasks, tileSize, tileSize, layerTasks.toString(), noopTaskCache(), stats))
    }
